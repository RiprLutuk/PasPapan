    ...
    @foreach ($educations as $education)
      <div class="card p-4">
          <div class="flex justify-between items-start mb-3">
              <h4 class="text-base font-bold text-gray-900 dark:text-white">{{ $education->name }}</h4>
          </div>
          <div class="grid grid-cols-2 gap-3 pt-3 border-t border-gray-100 dark:border-gray-700">
              <x-secondary-button wire:click="edit({{ $education->id }})" class="justify-center px-2 py-1">
                  <x-heroicon-o-pencil class="w-4 h-4" />
              </x-secondary-button>
              <x-danger-button wire:click="confirmDeletion({{ $education->id }}, '{{ $education->name }}')" class="justify-center px-2 py-1">
        ...
        @foreach ($educations as $education)
          <tr>
            ...
            <td class="relative flex justify-end gap-2 px-6 py-4">
              <x-button wire:click="edit({{ $education->id }})" class="px-2 py-1">
                <x-heroicon-o-pencil class="w-4 h-4" />
              </x-button>
              <x-danger-button wire:click="confirmDeletion({{ $education->id }}, '{{ $education->name }}')" class="px-2 py-1">
                <x-heroicon-o-trash class="w-4 h-4" />
              </x-danger-button>
            </td>
          </tr>
