<x-app-layout>
    <x-admin.page-shell
        ...
        :description="__('Adjust barcode details, coordinates, and attendance radius for this checkpoint.')"
    >
        ...
        <div class="overflow-hidden rounded-3xl border border-gray-100 bg-white shadow-xl dark:border-gray-700 dark:bg-gray-800">
            <div class="p-4 lg:p-6">
                <div class="mb-6 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-slate-50/80 p-4 dark:border-slate-700 dark:bg-slate-900/70 md:flex-row md:items-center md:justify-between">
                    <div>
                        <p class="text-sm font-semibold text-slate-900 dark:text-white">{{ $barcode->name }}</p>
                        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                            {{ $barcode->dynamic_enabled
                                ? __('Dynamic mode is active. Use the live display page to present the rotating QR at this checkpoint.')
                                : __('Static mode is active. You can enable dynamic mode below if you want the QR to rotate automatically.') }}
                        </p>
                    </div>
                        ...
                        <div class="flex flex-col gap-4 md:flex-row md:items-start md:gap-3">
                            ...
                            <div id="barcode-value-field" class="w-full">
                                ...
                                <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                                    {{ __('Wajib untuk barcode statis. Untuk mode dinamis, kode internal lama tetap dipakai dan QR dibuat dari Live Display.') }}
                                </p>
                            </div>
                            <div id="dynamic-barcode-value-info" class="hidden w-full rounded-2xl border border-emerald-200 bg-emerald-50/80 p-4 text-sm text-emerald-700 dark:border-emerald-900/50 dark:bg-emerald-950/30 dark:text-emerald-300">
                                {{ __('Mode dinamis aktif: Value Barcode disembunyikan. Kode internal tetap tersimpan, sedangkan QR yang discan berasal dari halaman Live Display dan berubah otomatis.') }}
                            </div>
                        </div>
                        ...
                        <div class="mt-4 flex gap-3">
                            <div class="w-full">
                                ...
                                <x-forms.input name="radius" id="radius" class="mt-1 block w-full" type="number"
                                    :value="old('radius') ?? $barcode->radius" placeholder="50 (meter)" />
                                @error('radius')
                        ...
                        <div class="mt-4 rounded-2xl border border-slate-200 bg-slate-50/80 p-4 dark:border-slate-700 dark:bg-slate-900/70">
                            <div class="flex items-start gap-3">
                                <input
                                    ...
                                    type="checkbox"
                                    value="1"
                                    @checked(old('dynamic_enabled', $barcode->dynamic_enabled))
                                    class="mt-1 rounded border-gray-300 text-primary-600 focus:ring-primary-500"
                                >
                                <div class="w-full">
                                    <x-forms.label for="dynamic_enabled" value="{{ __('Aktifkan barcode dinamis') }}" />
                                    <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                                        {{ __('Jika aktif, scan hanya akan menerima QR dinamis bernonce acak yang ditampilkan dari halaman display barcode.') }}
                                    </p>
                                    <p class="mt-2 text-xs text-amber-600 dark:text-amber-400">
                                        {{ __('Regenerate secret jika Anda ingin langsung membatalkan semua QR dinamis yang sebelumnya sempat tampil.') }}
                                    </p>
                                    ...
                                    <div class="mt-4 max-w-xs">
                                        ...
                                        <x-forms.input
                                            ...
                                            min="30"
                                            max="300"
                                            :value="old('dynamic_ttl_seconds', $barcode->dynamic_ttl_seconds ?: 60)"
                                        />
                                        <p class="mt-1 text-xs text-slate-500 dark:text-slate-400">
                                ...
                                <div class="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                                    <div>
                                        ...
                                        <p id="location-helper-status" class="mt-1 text-sm text-slate-500 dark:text-slate-400">
                                            {{ __('Use your current location or adjust latitude and longitude manually to reposition this checkpoint.') }}
                                        </p>
                                    </div>
                            ...
                            <div class="grid grid-cols-1 gap-3 md:grid-cols-2">
                                <div class="w-full">
                                    ...
                                    <x-forms.input name="lat" id="lat" class="mt-1 block w-full" type="text"
                                        :value="old('lat') ?? $barcode->latLng['lat']" placeholder="cth: -6.12345" />
                                    @error('lat')
                                ...
                                <div class="w-full">
                                    ...
                                    <x-forms.input name="lng" id="lng" class="mt-1 block w-full" type="text"
                                        :value="old('lng') ?? $barcode->latLng['lng']" placeholder="cth: 6.12345" />
                                    @error('lng')
